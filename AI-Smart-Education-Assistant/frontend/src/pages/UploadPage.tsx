...
import type { Document, Subject, UploadJob } from '@/types';
import {
  ROUTES,
  SUBJECT_COLORS,
  SUBJECT_ICONS,
...
import { Progress, UploadProgress as UploadProgressComp } from '@/components/ui/Progress';
import { Spinner } from '@/components/ui/Spinner';
import { NoDocuments } from '@/components/ui/EmptyState';
import { cn, formatFileSize, formatRelativeTime, generateId } from '@/utils/format';
...
interface PendingJob {
  ...
  type: Document['type'];
  progress: number;
  status: UploadJob['status'] | 'invalid';
  error?: string;
  subject?: string;
  invalidReason?: 'duplicate' | 'unsupported' | 'too-large' | 'empty';
}

const detectDocType = (file: File): Document['type'] => {
  const t = file.type;
...
const getInitialDocuments = (): Document[] => [
  ...
  { id: generateId(), name: 'Organic_Chemistry_Notes.pdf', type: 'pdf', size: 5400000, status: 'ready', uploadDate: new Date(Date.now() - 3600000 * 5).toISOString(), subject: 'Chemistry', pages: 72, favorite: false, lastOpened: new Date(Date.now() - 86400000).toISOString() },
  { id: generateId(), name: 'Physics_Waves.docx', type: 'doc', size: 890000, status: 'ready', uploadDate: new Date(Date.now() - 86400000 * 2).toISOString(), subject: 'Physics', pages: 18, favorite: false, lastOpened: new Date(Date.now() - 3600000 * 6).toISOString() },
  { id: generateId(), name: 'Mitochondria_Diagram.png', type: 'image', size: 2100000, status: 'ready', uploadDate: new Date(Date.now() - 86400000 * 3).toISOString(), subject: 'Biology', favorite: true, lastOpened: new Date(Date.now() - 86400000 * 3).toISOString() },
  { id: generateId(), name: 'Algorithms_Notes.txt', type: 'text', size: 145000, status: 'ready', uploadDate: new Date(Date.now() - 86400000 * 4).toISOString(), subject: 'Computer Science', pages: 12, favorite: false, lastOpened: new Date(Date.now() - 86400000 * 2).toISOString() },
  { id: generateId(), name: 'WWII_Timeline.pdf', type: 'pdf', size: 2800000, status: 'ready', uploadDate: new Date(Date.now() - 86400000 * 5).toISOString(), subject: 'History', pages: 34, favorite: false },
  { id: generateId(), name: 'Macro_Study_Guide.pdf', type: 'pdf', size: 4100000, status: 'ready', uploadDate: new Date(Date.now() - 86400000 * 6).toISOString(), subject: 'Economics', pages: 58, favorite: true },
  { id: generateId(), name: 'Hamlet_Analysis.docx', type: 'doc', size: 520000, status: 'ready', uploadDate: new Date(Date.now() - 86400000 * 7).toISOString(), subject: 'English Literature', pages: 22, favorite: false },
];
...
export const UploadPage: React.FC = () => {
  ...
  const [subjects, setSubjects] = useState<Subject[]>(storedSubjects.length > 0 ? storedSubjects : getDemoSubjects());
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      sizeUsed: totalBytes,
      processing: jobs.filter((j) => j.status === 'uploading' || j.status === 'processing').length,
  ...
  const validateFile = useCallback((file: File, existingDocs: Document[]): PendingJob | null => {
    const id = generateId();
    ...
    if (file.size === 0) {
      return { ...base, status: 'invalid', invalidReason: 'empty', error: 'File is empty (0 bytes). Please select a valid file.' };
    }
    ...
    if (!ACCEPTED_TYPES.includes(file.type) && !(file.name.endsWith('.docx') || file.name.endsWith('.doc'))) {
      if (base.type === 'other') {
        return {
          ...base,
          status: 'invalid',
    ...
    const isDup = existingDocs.some((d) => d.name === file.name && Math.abs(d.size - file.size) < 1024);
    if (isDup) {
      toast.warning('Duplicate detected', { description: `"${file.name}" appears to already exist.` });
    }
  ...
  const enqueueFiles = useCallback((files: FileList | null) => {
    ...
    const validJobs: PendingJob[] = [];
    const invalidJobs: PendingJob[] = [];
    ...
    if (validJobs.length > 0) {
      toast.success(`${validJobs.length} file${validJobs.length > 1 ? 's' : ''} added to queue`);
    }
  ...
  const startJob = useCallback((jobId: string) => {
    setJobs((prev) => prev.map((j) => (j.id === jobId ? { ...j, status: 'uploading', progress: 0 } : j)));
      ...
      setJobs((prev) => {
        ...
        if (nextProg >= 100) {
          ...
          if (shouldFail) {
            return prev.map((j) =>
              j.id === jobId
                ? { ...j, status: 'failed', progress: 92, error: 'Network error during upload. Please retry.' }
                : j
            );
          }
          ...
          setTimeout(() => {
            setJobs((j2) =>
              j2.map((j) => (j.id === jobId ? { ...j, status: 'processing' } : j))
            );
            setTimeout(() => {
              setJobs((j2) => {
                ...
                const newDoc: Document = {
                  ...
                  subject: job.subject,
                  pages: job.type === 'image' ? undefined : Math.floor(Math.random() * 80) + 5,
                  favorite: false,
                };
                setDocuments((docs) => [newDoc, ...docs]);
                toast.success(`${job.fileName} uploaded successfully`);
                return j2.map((j) => (j.id === jobId ? { ...j, status: 'done', progress: 100 } : j));
              });
            }, 1200 + Math.random() * 1200);
          }, 400);

          return prev.map((j) => (j.id === jobId ? { ...j, progress: 100 } : j));
        }

        return prev.map((j) => (j.id === jobId ? { ...j, progress: nextProg } : j));
      });
    }, 280);
  }, []);
  ...
  const cancelJob = useCallback((jobId: string) => {
    ...
    setJobs((prev) =>
      prev.map((j) =>
        (j.id === jobId && (j.status === 'uploading' || j.status === 'processing' || j.status === 'queued'))
          ? { ...j, status: 'cancelled' }
          : j
      )
    );
  ...
  const clearCompleted = useCallback(() => {
    setJobs((prev) => prev.filter((j) => j.status !== 'done' && j.status !== 'invalid' && j.status !== 'cancelled'));
  }, []);

  const setJobSubject = useCallback((jobId: string, subject: string) => {
    setJobs((prev) => prev.map((j) => (j.id === jobId ? { ...j, subject: subject || undefined } : j)));
  }, []);
  ...
  const onDrop = (e: React.DragEvent) => {
    ...
    setIsDragOver(false);
    if (!isOnline) {
      toast.error('Network disconnected', { description: 'Connect to the internet to upload files.' });
      return;
    }
  ...
  const handleRenameDoc = (doc: Document) => {
    const name = window.prompt('Rename document:', doc.name);
    if (name && name.trim()) {
      setDocuments((prev) => prev.map((d) => (d.id === doc.id ? { ...d, name: name.trim() } : d)));
      toast.success('Document renamed');
    }
  };

  const handleReplaceDoc = (doc: Document) => {
  ...
  const handlePreviewDoc = (doc: Document) => {
    ...
    navigate((ROUTES as any).library);
  };

  const filteredDocs = useMemo(() => {
    const base = [...documents].sort((a, b) => new Date(b.uploadDate).getTime() - new Date(a.uploadDate).getTime());
    ...
    >
        ...
        {!isOnline && (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-semibold text-warning-900 dark:text-warning-100">Network disconnected</h3>
      ...
      <motion.div variants={item}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Upload Center</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              onClick={() => navigate((ROUTES as any).library)}
            >
              Go to Library
            </Button>
          </div>
          ...
          <CardContent className="space-y-5">
            ...
            >
              ...
              <p className="mt-2 max-w-md text-sm text-slate-500 dark:text-slate-400">
                Up to {formatFileSize(MAX_FILE_SIZE)} each. Supports PDF, DOCX, images (PNG/JPG/GIF/WebP), TXT, and Markdown.
              </p>
              <Button
                type="button"
                variant="primary"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-sm">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  disabled={!jobs.some((j) => j.status === 'done' || j.status === 'invalid' || j.status === 'cancelled')}
                >
                  Clear Completed
                </Button>
                <Button
                  ...
                  onClick={cancelAll}
                  disabled={!jobs.some((j) => j.status === 'queued' || j.status === 'uploading' || j.status === 'processing')}
                >
                  Cancel All
                </Button>
              </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {jobs.filter((j) => j.status === 'queued').length} queued • {jobs.filter((j) => j.status === 'uploading' || j.status === 'processing').length} active • {jobs.filter((j) => j.status === 'done').length} done
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
...
interface JobRowProps {
  job: PendingJob;
  subjects: Subject[];
  onSetSubject: (jobId: string, subject: string) => void;
  onCancel: (jobId: string) => void;
  onRemove: (jobId: string) => void;
  onRetry: (jobId: string) => void;
}

const statusStyles: Record<NonNullable<PendingJob['status']>, string> = {
  queued: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300',
  uploading: 'bg-primary-50 text-primary-700 dark:bg-primary-950/40 dark:text-primary-400',
  processing: 'bg-warning-50 text-warning-700 dark:bg-warning-950/40 dark:text-warning-400',
  done: 'bg-accent-50 text-accent-700 dark:bg-accent-950/40 dark:text-accent-400',
  failed: 'bg-danger-50 text-danger-700 dark:bg-danger-950/40 dark:text-danger-400',
  cancelled: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
  invalid: 'bg-danger-50 text-danger-700 dark:bg-danger-950/40 dark:text-danger-400',
};

const statusIcons: Record<NonNullable<PendingJob['status']>, React.ReactNode> = {
...
const statusLabels: Record<NonNullable<PendingJob['status']>, string> = {
  queued: 'Queued',
  uploading: 'Uploading',
...
const JobRow: React.FC<JobRowProps> = ({ job, subjects, onSetSubject, onCancel, onRemove, onRetry }) => {
  const canCancel = job.status === 'queued' || job.status === 'uploading' || job.status === 'processing';
  const canRetry = job.status === 'failed' || job.status === 'cancelled';
  const canRemove = job.status === 'done' || job.status === 'cancelled' || job.status === 'invalid' || job.status === 'queued';
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-slate-900 dark:text-slate-100" title={job.fileName}>
                {job.fileName}
              </p>
          ...
          <div className="mt-3 flex flex-col gap-3">
            {(job.status === 'queued' || job.status === 'uploading' || job.status === 'processing' || job.status === 'done' || job.status === 'failed' || job.status === 'cancelled') && (
              <div className="flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1 text-xs">
                    <span className="text-slate-500 dark:text-slate-400">
                      {job.status === 'uploading' ? 'Uploading…' : job.status === 'processing' ? 'Processing…' : job.status === 'done' ? 'Complete' : job.status === 'queued' ? 'Waiting' : 'Progress'}
                    </span>
                    <span className="font-medium text-slate-700 dark:text-slate-300">{Math.round(job.progress)}%</span>
                  </div>
                  <Progress
                    value={job.progress}
                    size="sm"
                    color={job.status === 'done' ? 'accent' : job.status === 'failed' ? 'danger' : job.status === 'processing' ? 'warning' : 'primary'}
                  />
                </div>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {canRetry && (
                <Button size="sm" variant="outline" leftIcon={<RotateCcw className="h-3.5 w-3.5" />} onClick={() => onRetry(job.id)}>
                  Retry
                </Button>
              )}
              {canCancel && (
              ...
              {canRemove && (
                <Button
                  ...
                  aria-label="Remove"
                  className={cn(job.status === 'invalid' ? 'text-danger-500 hover:bg-danger-50 dark:hover:bg-danger-950/30' : 'text-slate-500')}
                >
                  <X className="h-4 w-4" />
                </Button>
              )}
...
interface CompletedDocCardProps {
  doc: Document;
  subjects: Subject[];
  onPreview: () => void;
  onRename: () => void;
...
const CompletedDocCard: React.FC<CompletedDocCardProps> = ({ doc, subjects, onPreview, onRename, onReplace, onDelete }) => {
      ...
      <Card hoverable padded={false} className="overflow-hidden group">
        <div className="relative aspect-[4/3] bg-gradient-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-900 flex items-center justify-center">
          ...
          {doc.subject && subject && (
            ...
            >
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: subject.color }} aria-hidden="true" />
              {doc.subject}
            </span>
          )}
          <span className={cn(
