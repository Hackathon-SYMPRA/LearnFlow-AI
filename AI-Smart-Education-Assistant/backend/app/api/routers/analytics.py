...
async def get_all_analyticss(
    ...
    objs = await analytics_service.get_by_user(current_user.id, skip=skip, limit=limit, filters=filters, sort=sort)
    return SuccessResponse(
        message="Analyticss retrieved successfully", 
...
async def get_analytics(id: str, current_user: UserInDB = Depends(get_current_user)):
    obj = await analytics_service.get_by_id(id, current_user.id)
    if not obj:
...
async def update_analytics(
    ...
    updated_obj = await analytics_service.update(id, current_user.id, update_data)
    if not updated_obj:
        raise HTTPException(status_code=404, detail="Analytics not found or update failed")
...
async def delete_analytics(id: str, current_user: UserInDB = Depends(get_current_user)):
    success = await analytics_service.delete(id, current_user.id)
    if not success:
        raise HTTPException(status_code=404, detail="Analytics not found or delete failed")
