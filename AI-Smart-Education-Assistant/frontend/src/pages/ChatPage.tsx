...
type ChatDateGroup = 'Today' | 'Yesterday' | 'Previous 7 Days' | 'Older';

const groupDateKey = (d: Date): ChatDateGroup => {
  const now = new Date();
...
const SAMPLE_CITATIONS_1: Citation[] = [
  ...
  },
  {
    ...
    confidence: 0.87,
    text:
      "Theorem 3.10 (Squeeze Theorem). Suppose that g(x) \u2264 f(x) \u2264 h(x) for all x in some deleted neighbourhood of a, and that lim x\u2192a g(x) = L and lim x\u2192a h(x) = L. Then lim x\u2192a f(x) exists and equals L. The Squeeze Theorem is an indispensable tool for limits involving trigonometric functions, especially the classic limit of sin(x)/x as x tends to zero.",
  },
  {
    ...
    confidence: 0.74,
    text:
      "L'H\u00f4pital's Rule applies to indeterminate forms 0/0 and \u221e/\u221e. Before applying the rule, always confirm that the form is indeed indeterminate. A common student error is differentiating the expression as a fraction via the quotient rule; the rule instead calls for differentiating numerator and denominator separately.",
  },
];
...
const SAMPLE_CITATIONS_2: Citation[] = [
  {
    documentName: 'Biology_Photosynthesis_Overview.pdf',
    page: 12,
    confidence: 0.9,
    text:
      'Photosynthesis may be summarised by the equation 6 CO2 + 6 H2O + light -> C6H12O6 + 6 O2. The process occurs in two coupled stages: the light-dependent reactions in the thylakoids, and the Calvin cycle in the stroma of the chloroplast.',
  },
  {
    documentName: 'Biology_Photosynthesis_Overview.pdf',
    page: 18,
    confidence: 0.82,
    text:
      'Chlorophyll a and chlorophyll b absorb light primarily in the blue and red regions of the visible spectrum; they reflect green light, which is why plant leaves typically appear green to the human eye.',
  },
];
...
const subjectColorFor = (subject?: string): string => {
...
const renderInline = (raw: string): string => {
  ...
  html = html.replace(
    boldRe,
    (_m, text) => `<strong class="font-semibold text-slate-900 dark:text-slate-100">${text}</strong>`
  );
  html = html.replace(
    italicRe,
...
const renderMarkdown = (raw: string): string => {
  ...
  const pushPara = (buf: string[]) => {
    ...
    if (joined) {
      out.push(`<p class="mb-3 leading-relaxed text-slate-800 dark:text-slate-200">${renderInline(joined)}</p>`);
    }
    buf.length = 0;
  };
...
interface ReactionsState {
  [msgId: string]: 'like' | 'dislike' | null;
}
...
export const ChatPage: React.FC = () => {
  ...
  const [procStep, setProcStep] = useState<0 | 1 | 2 | 3>(0);
  ...
  const { pinned, grouped } = useMemo(() => {
    ...
    const groups: Record<ChatDateGroup, ChatSession[]> = {
      Today: [],
      Yesterday: [],
      'Previous 7 Days': [],
      Older: [],
    ...
    (Object.keys(groups) as ChatDateGroup[]).forEach((k) => {
      groups[k].sort(
        (a, b) =>
  ...
  const commitRename = useCallback(async () => {
    ...
    setCurrentSession((prev) => (prev && prev.id === id ? { ...prev, title } : prev));
  ...
  const startInlineTitleEdit = useCallback((current: string) => {
  ...
  const toggleReaction = useCallback(
    (msgId: string, reaction: 'like' | 'dislike') => {
      setReactions((prev) => ({
        ...prev,
  ...
  const handleSend = useCallback(
    async (
      ...
      let stopped = false;
      (window as unknown as { __chatStop?: () => void }).__chatStop = () => {
        stopped = true;
      };
            ...
            prev.map((s) =>
              s.id === session!.id
                ? {
                    ...s,
                    lastMessage:
                      appended.slice(0, 80) || s.lastMessage,
                    updatedAt: new Date().toISOString(),
  ...
  const handleStop = useCallback(() => {
    if (typeof window !== 'undefined') {
      const w = window as unknown as { __chatStop?: () => void };
  ...
  const confidenceFromCitations = (
    citations?: Citation[]
  ): { level: 'High' | 'Medium' | 'Low'; value: number } => {
    if (!citations || citations.length === 0)
      return { level: 'Medium', value: 0.72 };
    const avg =
      citations.reduce(
        (sum, c) => sum + (typeof c.confidence === 'number' ? c.confidence : 0.7),
        0
      ) / citations.length;
  ...
  const renderSidebarSessionRow = (s: ChatSession, pinned = false) => {
              ...
              {menuOpenSession === s.id ? (
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setMenuOpenSession((prev) => (prev === s.id ? null : s.id));
                    }}
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        ...
        <section>
          ...
          {(['Today', 'Yesterday', 'Previous 7 Days', 'Older'] as ChatDateGroup[]).map(
            (g) =>
              grouped[g].length > 0 && (
            ...
            <div className="h-full flex flex-col items-center justify-center px-4 py-10 text-center">
              ...
              >
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                  Start a conversation using your uploaded study materials. Answers include citations, related questions, and exam tips.
                </p>
                        ...
                        <div className="min-w-0 flex-1 max-w-[92%] md:max-w-[82%]">
                          {isStreamingMsg && procStep < 3 ? (
                            <div className="rounded-2xl rounded-tl-md border-l-4 border-primary-500 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm px-4 py-3 mb-2">
                              <ProcessingStepper step={procStep as 0 | 1 | 2 | 3} />
                            </div>
                          ) : (
                            <>
                              <div
                                                ...
                                                <div className="min-w-0 flex-1">
                                                  ...
                                                  <p className="mt-0.5 text-[10px] text-slate-500 dark:text-slate-400">
                                                    {c.page
                                                      ...
                                                      : 'Source'}
                                                    {' \u2022 '}
                                                    {tone.level} {Math.round(tone.value * 100)}%
                                                  </p>
        ...
        <div className="border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 md:px-6 py-3">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="mt-1.5 text-center text-[10px] text-slate-400 dark:text-slate-500">
              Answers may include AI-generated content. Always verify with your notes.
            </p>
          </div>
        </div>
      </main>
