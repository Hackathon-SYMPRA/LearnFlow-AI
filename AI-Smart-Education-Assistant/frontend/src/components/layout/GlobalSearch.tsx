...
import { cn, debounce, formatRelativeTime, truncateText } from '@/utils/format';

type SearchTab = 'all' | 'document' | 'subject' | 'chat' | 'flashcard' | 'quiz';
...
const MOCK_DOCUMENTS: SearchResultItem[] = [
  { type: 'document', id: 'doc-1', title: 'Quantum Mechanics - Chapter 4 Notes', excerpt: 'Wave functions and Schrödinger equation fundamentals with examples...', subject: 'Physics', meta: '3 days ago', badge: 'PDF' },
  { type: 'document', id: 'doc-2', title: 'Calculus II Integration Techniques', excerpt: 'Integration by parts, trig substitutions, and partial fractions explained...', subject: 'Mathematics', meta: '1 week ago', badge: 'PDF' },
  { type: 'document', id: 'doc-3', title: 'Organic Chemistry Reactions Summary', excerpt: 'SN1, SN2, E1, E2 reaction mechanisms and stereochemistry...', subject: 'Chemistry', meta: '2 weeks ago', badge: 'DOC' },
];

const MOCK_SUBJECTS: SearchResultItem[] = SUBJECTS.slice(0, 6).map((name, i) => ({
...
const MOCK_CHATS: SearchResultItem[] = [
  { type: 'chat', id: 'chat-1', title: 'Help with thermodynamics', excerpt: 'Can you explain the second law and entropy in simple terms?', meta: '2 hours ago' },
  { type: 'chat', id: 'chat-2', title: 'Algebra linear equations practice', excerpt: 'Show me step by step how to solve systems of equations...', meta: 'Yesterday' },
];

const MOCK_FLASHCARDS: SearchResultItem[] = [
  { type: 'flashcard', id: 'fc-1', title: 'Photosynthesis Process', excerpt: 'What is the primary function of chlorophyll?', subject: 'Biology', meta: '24 cards' },
  { type: 'flashcard', id: 'fc-2', title: 'Programming Data Structures', excerpt: 'Difference between stack and queue?', subject: 'Computer Science', meta: '42 cards' },
];

const MOCK_QUIZZES: SearchResultItem[] = [
  { type: 'quiz', id: 'quiz-1', title: 'Physics Midterm Practice', excerpt: 'Covers kinematics, forces, energy', badge: '92%', meta: '10 questions · 3 days ago' },
  { type: 'quiz', id: 'quiz-2', title: 'History - Renaissance Era', excerpt: 'Art, science, and humanism topics', badge: '78%', meta: '15 questions · 1 week ago' },
];

const getTypeIcon = (type: SearchResultItem['type'], metaIcon?: string) => {
  switch (type) {
    case 'document':
      return FileText;
    case 'subject':
...
const getTypeIconBg = (type: SearchResultItem['type']) => {
  switch (type) {
    case 'document':
...
export const GlobalSearch: React.FC = () => {
  ...
  const filteredResults = useMemo(() => {
    ...
    const allItems = [...MOCK_DOCUMENTS, ...MOCK_SUBJECTS, ...MOCK_CHATS, ...MOCK_FLASHCARDS, ...MOCK_QUIZZES];
    ...
    const filtered = items.filter((item) => {
      const hay = `${item.title} ${item.excerpt || ''} ${item.subject || ''}`.toLowerCase();
  ...
  const flatResults = useMemo(() => {
    ...
    const g = (filteredResults.grouped || {}) as Record<string, SearchResultItem[]>;
    const order: SearchResultItem['type'][] = ['document', 'subject', 'chat', 'flashcard', 'quiz'];
            ...
            <div className="max-h-[55vh] overflow-y-auto">
              {!filteredResults.hasQuery ? (
                ...
                <div className="p-2 sm:p-3 space-y-4">
                  {(['document', 'subject', 'chat', 'flashcard', 'quiz'] as const).map((type) => {
                    const group = (filteredResults.grouped as Record<string, SearchResultItem[]>)?.[type];
                    ...
                    return (
                      <div key={type}>
                        ...
                        <ul className="space-y-1">
                          {group.map((item) => {
                            const flatIdx = flatResultsRef.current.findIndex((r) => r.id === item.id && r.type === item.type);
                            ...
                            return (
                              <li key={`${item.type}-${item.id}`}>
                                ...
                                >
                                  <div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl', getTypeIconBg(item.type))}>
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    {item.excerpt && (
                                      <p className="mt-1 line-clamp-1 text-xs text-slate-500 dark:text-slate-400">
                                        {highlightText(truncateText(item.excerpt, 90), debouncedQuery)}
                                      </p>
                                    )}
                                  ...
                                  <ChevronRight className={cn('h-4 w-4 shrink-0 self-center transition-colors', isHighlighted ? 'text-primary-500' : 'text-slate-300 dark:text-slate-600')} />
                                </button>
                              </li>
                            );
                          })}
            ...
            <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-800 px-4 py-2.5 sm:px-5 bg-slate-50/60 dark:bg-slate-800/40">
              <div className="flex items-center gap-3 text-[11px] text-slate-500 dark:text-slate-400">
                <span className="inline-flex items-center gap-1">
                  <kbd className="rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-1.5 py-0.5 font-mono text-[10px]">↑↓</kbd>
                  Navigate
                </span>
                <span className="inline-flex items-center gap-1">
                  <kbd className="rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-1.5 py-0.5 font-mono text-[10px]">↵</kbd>
                ...
                <span className="inline-flex items-center gap-1">
                  <kbd className="rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-1.5 py-0.5 font-mono text-[10px]">Esc</kbd>
                  Close
                </span>
              </div>
              <div className="hidden sm:flex items-center gap-1 text-[11px] text-slate-400 dark:text-slate-500">
                <Search className="h-3 w-3" />
                <span>Press <kbd className="rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-1 font-mono">⌘K</kbd> anytime</span>
              </div>
            </div>
          </motion.div>
        </motion.div>
