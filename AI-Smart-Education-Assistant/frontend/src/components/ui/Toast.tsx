...
const iconMap: Record<ToastType, React.ReactNode> = {
  success: <CheckCircle2 className="h-5 w-5 text-accent-600 dark:text-accent-400" aria-hidden="true" />,
  error: <XCircle className="h-5 w-5 text-danger-600 dark:text-danger-400" aria-hidden="true" />,
  warning: <AlertTriangle className="h-5 w-5 text-warning-600 dark:text-warning-400" aria-hidden="true" />,
  info: <Info className="h-5 w-5 text-primary-600 dark:text-primary-400" aria-hidden="true" />,
};
      ...
      (id) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-slate-900 dark:text-slate-100">{message}</p>
            {options?.description && (
              <p className="mt-0.5 text-sm text-slate-600 dark:text-slate-400">{options.description}</p>
            )}
            {options?.action && (
              <button
                onClick={() => {
