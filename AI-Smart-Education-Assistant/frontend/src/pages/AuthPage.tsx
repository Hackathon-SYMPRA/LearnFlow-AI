...
const SSOButton: React.FC<SSOButtonProps> = ({ provider, icon }) => {
  return (
...
export const AuthPage: React.FC = () => {
  ...
  const fillDemo = () => {
    if (mode === 'login') {
      loginForm.setValue('email', DEMO_EMAIL, { shouldValidate: true, shouldDirty: true });
      loginForm.setValue('password', DEMO_PASSWORD, { shouldValidate: true, shouldDirty: true });
      toast.info('Demo credentials filled', { description: 'Click Sign In to explore.' });
    } else {
      registerForm.setValue('name', 'Demo Student', { shouldValidate: true, shouldDirty: true });
      registerForm.setValue('email', DEMO_EMAIL, { shouldValidate: true, shouldDirty: true });
      registerForm.setValue('password', DEMO_PASSWORD, { shouldValidate: true, shouldDirty: true });
      registerForm.setValue('confirmPassword', DEMO_PASSWORD, { shouldValidate: true, shouldDirty: true });
      toast.info('Demo credentials filled', { description: 'Click Create Account to explore.' });
    }
  };
  ...
  const onLogin = async (data: LoginInput) => {
    ...
    try {
      ...
      const redirect = (location.state as { from?: Location })?.from?.pathname ?? ROUTES.dashboard;
  ...
  const onRegister = async (data: RegisterInput) => {
    ...
    try {
      ...
      toast.success('Account created!', { description: 'Welcome to EduMind AI.' });
      navigate(ROUTES.dashboard, { replace: true });
    } catch (error) {
      ...
      toast.error(message, {
        description: isNetwork ? 'Check your connection and try again.' : 'Please try a different email.',
      });
    }
  };
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-primary-600 via-primary-500 to-secondary-600 text-white p-12 flex-col">
        ...
        <div className="mt-auto space-y-8 max-w-lg">
          ...
          <p className="text-lg text-white/80">
            Upload any study material, ask questions, generate quizzes, flashcards, and
            personalized study plans — all grounded in your content.
          </p>
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-8">
        ...
        >
          <div className="lg:hidden flex items-center gap-3 mb-8">
            ...
            <div>
              <p className="text-xl font-bold text-slate-900 dark:text-slate-100">EduMind AI</p>
          ...
          {networkError && (
            <motion.div
              ...
              role="alert"
            >
              <WifiOff className="h-5 w-5 text-danger-600 dark:text-danger-400 shrink-0 mt-0.5" aria-hidden="true" />
              <div className="text-sm">
                <p className="font-medium text-danger-800 dark:text-danger-200">Connection error</p>
                <p className="mt-0.5 text-danger-700 dark:text-danger-300">{networkError}</p>
              </div>
            </motion.div>
          )}
            ...
            {mode === 'login' ? (
              ...
              <form onSubmit={registerForm.handleSubmit(onRegister)} className="space-y-4" noValidate>
                <Input
                  ...
                  leftIcon={<UserCircle className="h-4 w-4" aria-hidden="true" />}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1">
                  <p className="text-slate-600 dark:text-slate-300 flex items-center gap-1.5">
                    <CheckCircle2 className="h-3.5 w-3.5 text-accent-600 dark:text-accent-400" aria-hidden="true" />
