...
import { generateQuizSchema, type GenerateQuizInput } from '@/utils/validation';
...
type QuizState = 'config' | 'quiz' | 'review' | 'results';
type QuestionType = 'mcq' | 'true-false' | 'fill-blanks' | 'short-answer';
...
const MOCK_DOCUMENTS: MockDocument[] = [
  { id: 'doc-1', name: 'Calculus_Chapter5_Integration.pdf', subject: 'Mathematics', pages: 42, type: 'pdf' },
  { id: 'doc-2', name: 'Algebra_Linear_Transformations.docx', subject: 'Mathematics', pages: 28, type: 'doc' },
  { id: 'doc-3', name: 'Physics_Mechanics_Waves.pdf', subject: 'Physics', pages: 56, type: 'pdf' },
  { id: 'doc-4', name: 'Electromagnetism_Notes.pdf', subject: 'Physics', pages: 38, type: 'pdf' },
  { id: 'doc-5', name: 'Organic_Chemistry_Reactions.pdf', subject: 'Chemistry', pages: 64, type: 'pdf' },
  { id: 'doc-6', name: 'Periodic_Table_Trends.docx', subject: 'Chemistry', pages: 19, type: 'doc' },
  { id: 'doc-7', name: 'Cell_Biology_Mitosis.pdf', subject: 'Biology', pages: 33, type: 'pdf' },
  { id: 'doc-8', name: 'Data_Structures_Algorithms.pdf', subject: 'Computer Science', pages: 88, type: 'pdf' },
  { id: 'doc-9', name: 'World_War_II_History.pdf', subject: 'History', pages: 51, type: 'pdf' },
  { id: 'doc-10', name: 'Macroeconomics_Keynes.docx', subject: 'Economics', pages: 24, type: 'doc' },
];

const TOPIC_SUGGESTIONS: Record<string, string[]> = {
  Mathematics: ['Integration Techniques', 'Linear Algebra', 'Differential Equations', 'Probability Theory'],
  Physics: ['Newtonian Mechanics', 'Electromagnetic Waves', 'Quantum Basics', 'Thermodynamics'],
  Chemistry: ['Organic Reactions', 'Chemical Bonds', 'Periodic Trends', 'Stoichiometry'],
  Biology: ['Cell Division', 'Genetics', 'Ecosystems', 'Human Anatomy'],
  'Computer Science': ['Data Structures', 'Sorting Algorithms', 'Dynamic Programming', 'Graph Theory'],
  History: ['World War II', 'Industrial Revolution', 'Ancient Rome', 'Renaissance'],
  Geography: ['Plate Tectonics', 'Climate Systems', 'Urbanization', 'Biomes'],
  English: ['Grammar & Syntax', 'Literary Devices', 'Essay Writing', 'Poetry Analysis'],
  Economics: ['Microeconomics', 'Macroeconomics', 'Game Theory', 'International Trade'],
  Psychology: ['Cognitive Development', 'Behavioral Theory', 'Memory Systems', 'Social Psychology'],
};

const RECENT_QUIZZES: RecentQuiz[] = [
  { id: 'rq-1', subject: 'Mathematics', score: 82, questionsCount: 20, date: new Date(Date.now() - 86400000).toISOString(), title: 'Calculus Chapter 5 Quiz' },
  { id: 'rq-2', subject: 'Physics', score: 74, questionsCount: 15, date: new Date(Date.now() - 86400000 * 2).toISOString(), title: 'Mechanics Review' },
  { id: 'rq-3', subject: 'Chemistry', score: 91, questionsCount: 25, date: new Date(Date.now() - 86400000 * 4).toISOString(), title: 'Organic Reactions' },
  { id: 'rq-4', subject: 'Computer Science', score: 66, questionsCount: 30, date: new Date(Date.now() - 86400000 * 7).toISOString(), title: 'Data Structures' },
];

const QUESTION_LETTERS = ['A', 'B', 'C', 'D'];

const ProcessingStepper: React.FC<{ steps: ProcessingStep[] }> = ({ steps }) => (
  <div className="flex flex-col gap-2 rounded-xl bg-slate-50 dark:bg-slate-800/50 p-4">
    {steps.map((step, idx) => (
      <div key={step.label} className="flex items-center gap-3">
        ...
        >
          {step.done ? <Check className="h-3.5 w-3.5" /> : <span className="text-xs font-semibold">{idx + 1}</span>}
        </div>
        <span
          className={cn(
            'text-sm',
            step.done ? 'text-slate-700 dark:text-slate-300' : 'text-slate-500 dark:text-slate-400'
          )}
        >
          {step.label}
        </span>
...
const generateMockQuestions = (config: GenerateQuizInput): FullQuestion[] => {
...
const getRank = (score: number): { tier: string; color: string; min: number } => {
  if (score >= 90) return { tier: 'Platinum', color: 'from-cyan-400 via-blue-500 to-indigo-600', min: 90 };
  if (score >= 75) return { tier: 'Gold', color: 'from-amber-400 via-orange-500 to-orange-600', min: 75 };
  if (score >= 50) return { tier: 'Silver', color: 'from-slate-300 via-slate-400 to-slate-500', min: 50 };
  return { tier: 'Bronze', color: 'from-orange-400 via-amber-600 to-amber-800', min: 0 };
};
...
export const QuizPage: React.FC = () => {
  ...
  const runGenerationSteps = async (config: GenerateQuizInput) => {
    ...
    for (let i = 0; i < stepTimings.length; i++) {
      ...
      setProcessingSteps((prev) => prev.map((s, idx) => (idx <= i ? { ...s, done: true } : s)));
    }
  ...
  const topicSuggestions = watchedSubject ? TOPIC_SUGGESTIONS[watchedSubject]?.slice(0, 3) ?? [] : [];
  ...
  const calculateResults = () => {
    ...
    questions.forEach((q) => {
      ...
      if (q.type === 'mcq') {
        ...
        isCorrect = correctArr.every((c, i) => arr[i]?.toLowerCase().trim() === c.toLowerCase());
    ...
    const scorePct = questions.length === 0 ? 0 : Math.round((correct / questions.length) * 100);
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-primary-100 max-w-2xl">
              Configure your quiz parameters below. The AI will generate targeted questions based on your documents,
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Choose documents to base your quiz on</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 max-h-[340px] overflow-y-auto pr-1">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-slate-500 dark:text-slate-400">{doc.pages} pages</span>
                        </div>
                      </div>
                    </label>
                  );
                ...
                {filteredDocs.length === 0 && (
                  <div className="col-span-full text-center py-6 text-sm text-slate-500">No documents found</div>
                )}
              </div>
              {errors.documentIds && (
                <p className="text-sm text-danger-600 dark:text-danger-400">{errors.documentIds.message}</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Subject, topic, and question settings</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div>
                <label className="label-base mb-1.5" htmlFor="subject">Subject</label>
                <Controller
                  name="subject"
                  control={control}
                  render={({ field }) => (
                ...
                />
                {errors.subject && <p className="mt-1 text-sm text-danger-600 dark:text-danger-400">{errors.subject.message}</p>}
              </div>

              <div>
                <label className="label-base mb-1.5" htmlFor="topic">Chapter / Topic</label>
                <input
                  id="topic"
                  {...register('topic')}
                ...
                {errors.topic && <p className="mt-1 text-sm text-danger-600 dark:text-danger-400">{errors.topic.message}</p>}
              </div>
              ...
              <div>
                ...
                {errors.questionCount && (
                  <p className="mt-1 text-sm text-danger-600 dark:text-danger-400">{errors.questionCount.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {(['mcq', 'true-false', 'fill-blanks', 'short-answer'] as QuestionType[]).map((qt) => {
                ...
                {errors.questionTypes && (
                  <p className="mt-2 text-sm text-danger-600 dark:text-danger-400">{errors.questionTypes.message}</p>
                )}
              </div>
              ...
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 bg-slate-50/50 dark:bg-slate-800/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Enable Exam Mode</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                        Adds a countdown timer and auto-submits when time expires
                      </p>
                    </div>
                  </div>
                  <Controller
                    ...
                    render={({ field }) => (
                      <button
                        ...
                        className={cn(
                          ...
                          field.value ? 'bg-primary-600' : 'bg-slate-200 dark:bg-slate-700'
                        )}
                      >
                        <span
                          className={cn(
                  ...
                  {watchedExamMode && (
                    ...
                    >
                      <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
                        <div>
                          <label className="label-base mb-1.5">Timer Duration</label>
                          <Controller
                            name="examDuration"
                            control={control}
                            render={({ field }) => (
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                          ...
                          Your quiz will be automatically submitted when the timer expires.
                        </div>
                      </div>
                    </motion.div>
                  )}
              ...
              {RECENT_QUIZZES.map((rq) => (
                ...
                >
                  <div
                    ...
                    style={{ backgroundColor: `${SUBJECT_COLORS[SUBJECTS.indexOf(rq.subject) % SUBJECT_COLORS.length]}20`, color: SUBJECT_COLORS[SUBJECTS.indexOf(rq.subject) % SUBJECT_COLORS.length] }}
                  >
                    <BookOpen className="h-5 w-5" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-slate-900 dark:text-slate-100 truncate">{rq.title}</p>
                      <span
                        className={cn(
                          'text-xs font-semibold',
                          rq.score >= 75 ? 'text-accent-600 dark:text-accent-400' : rq.score >= 50 ? 'text-warning-600 dark:text-warning-400' : 'text-danger-600 dark:text-danger-400'
                        )}
                      >
                        {rq.score}%
                      </span>
  ...
  const renderQuizScreen = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      <Card className="p-4 sm:p-5">
        ...
        <div className="mt-4">
          <Progress value={answeredCount} max={questions.length} color="primary" />
        </div>
      </Card>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        {question.type === 'mcq' ? 'MCQ' : question.type === 'true-false' ? 'True / False' : question.type === 'fill-blanks' ? 'Fill Blanks' : 'Short Answer'}
                      </span>
                      <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 text-balance">
                        <span className="text-primary-600 dark:text-primary-400 mr-2">#{currentQ + 1}</span>
                        {question.question}
                      </h2>
                    </div>
                    <button
                  ...
                  {question.hasImage && (
                    <div className="flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 h-40 border border-dashed border-slate-300 dark:border-slate-700">
                      <div className="text-center">
                        ...
                        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Question reference image</p>
                      </div>
                    </div>
                  )}
                      ...
                      {question.options.map((opt, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className={cn('text-sm', selected ? 'text-primary-800 dark:text-primary-200 font-medium' : 'text-slate-700 dark:text-slate-300')}>
                              {opt}
                            </span>
                          </button>
                        );
                      ...
                      {[true, false].map((val) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              {val ? <Check className="h-6 w-6" /> : <X className="h-6 w-6" />}
                            </div>
                            <span className={cn('text-lg font-bold', selected ? (val ? 'text-accent-700 dark:text-accent-400' : 'text-danger-700 dark:text-danger-400') : 'text-slate-600 dark:text-slate-400')}>
                              {val ? 'True' : 'False'}
                            </span>
                          </button>
                        );
                  ...
                  {question.type === 'fill-blanks' && question.blanks && (
                    <div className="space-y-3">
                      <p className="text-sm text-slate-600 dark:text-slate-400">Fill in the blanks below:</p>
                  ...
                  {question.type === 'short-answer' && (
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between text-xs text-slate-500">
                        ...
                        <span>{((answers[question.id] as string) || '').split(/\s+/).filter(Boolean).length} words</span>
                      </div>
                    </div>
                  )}
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-5 gap-1.5">
              {questions.map((q, idx) => {
                const answered = answers[q.id] !== undefined && answers[q.id] !== '' && !(Array.isArray(answers[q.id]) && (answers[q.id] as unknown[]).every((v) => !v));
            ...
            <div className="mt-4 space-y-2 text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-600 dark:text-slate-400">Seen / Unanswered</span>
              </div>
            </div>
  ...
  const getQuestionResult = (q: FullQuestion): { correct: boolean; userAnswerText: string; correctAnswerText: string } => {
    const ua = answers[q.id];
    if (q.type === 'mcq') {
      const uaIdx = typeof ua === 'number' ? ua : -1;
      const caIdx = q.correctAnswer as any;
      return {
        correct: uaIdx === caIdx,
        userAnswerText: uaIdx >= 0 && q.options ? `${QUESTION_LETTERS[uaIdx]}. ${q.options[uaIdx]}` : 'No answer',
        correctAnswerText: q.options ? `${QUESTION_LETTERS[caIdx]}. ${q.options[caIdx]}` : '',
      };
    }
    if (q.type === 'true-false') {
      const uaVal = ua as boolean | undefined;
      const caVal = q.correctAnswer as boolean;
      return {
        correct: uaVal === caVal,
        userAnswerText: uaVal === undefined ? 'No answer' : uaVal ? 'True' : 'False',
        correctAnswerText: caVal ? 'True' : 'False',
      };
    }
    if (q.type === 'fill-blanks') {
      const uaArr = (Array.isArray(ua) ? ua : []) as string[];
      const caArr = q.correctAnswer as string[];
      const correct = caArr.every((c, i) => uaArr[i]?.toLowerCase().trim() === c.toLowerCase());
      return {
        correct,
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
        <Card className="p-4 sm:p-5">
          ...
          <div className="mt-4">
            <Progress value={currentQ + 1} max={questions.length} color="primary" />
          </div>
        </Card>
                ...
                >
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        {q.type === 'mcq' ? 'MCQ' : q.type === 'true-false' ? 'True / False' : q.type === 'fill-blanks' ? 'Fill Blanks' : 'Short Answer'}
                      </span>
                      <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 text-balance">
                        <span className="text-primary-600 dark:text-primary-400 mr-2">#{currentQ + 1}</span>
                        {q.question}
                  ...
                  {q.type === 'mcq' && q.options && (
                    <div className="space-y-2.5">
                      {q.options.map((opt, idx) => {
                        const isCorrect = idx === (q.correctAnswer as any);
                        const isUser = answers[q.id] === idx;
                        let stateClass = '';
                        if (isCorrect) stateClass = 'border-accent-500 bg-accent-50 dark:bg-accent-950/40 dark:border-accent-400';
                        else if (isUser && !isCorrect) stateClass = 'border-danger-500 bg-danger-50 dark:bg-danger-950/40 dark:border-danger-400';
                        else stateClass = 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900';
                        return (
                          <div
                            key={idx}
                            className={cn(
                          ...
                          >
                            ...
                            >
                              {isCorrect && !isUser ? <Check className="h-5 w-5" /> : isUser && !isCorrect ? <X className="h-5 w-5" /> : QUESTION_LETTERS[idx]}
                            </span>
                  ...
                  {q.type === 'true-false' && (
                    <div className="grid grid-cols-2 gap-4">
                      {[true, false].map((val) => {
                        ...
                        let stateClass = '';
                        if (isCorrect) stateClass = val ? 'border-accent-500 bg-accent-50 dark:bg-accent-950/40 dark:border-accent-400' : 'border-accent-500 bg-accent-50 dark:bg-accent-950/40 dark:border-accent-400';
                        else if (isUser && !isCorrect) stateClass = 'border-danger-500 bg-danger-50 dark:bg-danger-950/40 dark:border-danger-400';
                        else stateClass = 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900';
                        return (
                          <div
                            key={String(val)}
                            className={cn('flex items-center justify-center gap-3 p-5 rounded-xl border-2', stateClass)}
                          >
                            <div
                              className={cn(
                                'flex h-11 w-11 items-center justify-center rounded-full',
                            ...
                            <span className="text-lg font-bold">{val ? 'True' : 'False'}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  {q.type === 'fill-blanks' && (
                    <div className="space-y-3">
                      <div className="rounded-xl p-4 bg-slate-50 dark:bg-slate-800/50 space-y-2">
                        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Your Answers</p>
                        <p className="text-sm text-slate-700 dark:text-slate-300">{result.userAnswerText || 'No answer provided'}</p>
                      </div>
                      <div className="rounded-xl p-4 bg-accent-50 dark:bg-accent-950/30 space-y-2 border border-accent-200 dark:border-accent-900">
                        <p className="text-xs font-semibold text-accent-700 dark:text-accent-400 uppercase tracking-wide">Correct Answers</p>
                        <p className="text-sm text-accent-800 dark:text-accent-300 font-medium">{result.correctAnswerText}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {q.type === 'short-answer' && (
                    <div className="space-y-3">
                      <div className="rounded-xl p-4 bg-slate-50 dark:bg-slate-800/50 space-y-2">
                        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Your Answer</p>
                        <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{result.userAnswerText || 'No answer provided'}</p>
                      </div>
                      <div className="rounded-xl p-4 bg-accent-50 dark:bg-accent-950/30 space-y-2 border border-accent-200 dark:border-accent-900">
                        <p className="text-xs font-semibold text-accent-700 dark:text-accent-400 uppercase tracking-wide">Acceptable Response</p>
                        <p className="text-sm text-accent-800 dark:text-accent-300">{result.correctAnswerText}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="rounded-xl border border-secondary-200 dark:border-secondary-800 bg-secondary-50/50 dark:bg-secondary-950/20 p-4 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        {result.correct ? 'Strong Topic' : 'Weak Topic'}: {q.topic}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2.5 pt-2 border-t border-slate-200 dark:border-slate-800">
                    <Button
                      variant="secondary"
                      onClick={() => setCurrentQ((qIdx) => Math.max(0, qIdx - 1))}
                      disabled={currentQ === 0}
                      leftIcon={<ChevronLeft className="h-4 w-4" />}
                    >
                      Previous
                    ...
                    <Button
                      variant="primary"
                      onClick={() => setCurrentQ((qIdx) => Math.min(questions.length - 1, qIdx + 1))}
                      rightIcon={<ChevronRight className="h-4 w-4" />}
                    >
                      Next
                    </Button>
  ...
  const renderResultsScreen = () => {
    const { correct, scorePct, skipped, weakTopics, strongTopics } = calculateResults();
    const wrong = questions.length - correct - skipped;
    const rank = getRank(scorePct);
    const timeTakenSec = examMode ? Math.max(0, totalTime - timeLeft) : Math.round(answeredCount * 45);
    ...
    const aiSuggestions = [
      { icon: BookOpen, text: `Revise ${weakTopics[0]?.topic ?? 'chapter 3'} concepts thoroughly`, color: 'bg-primary-50 text-primary-700 dark:bg-primary-950/40 dark:text-primary-400' },
      { icon: Brain, text: `Practice more MCQs on ${weakTopics[1]?.topic ?? 'key topics'} to improve speed`, color: 'bg-secondary-50 text-secondary-700 dark:bg-secondary-950/40 dark:text-secondary-400' },
      { icon: Sparkles, text: 'Generate a targeted quiz on incorrect questions', color: 'bg-accent-50 text-accent-700 dark:bg-accent-950/40 dark:text-accent-400' },
      { icon: Clock, text: 'Spend 20 min daily reviewing flashcards for weak areas', color: 'bg-warning-50 text-warning-700 dark:bg-warning-950/40 dark:text-warning-400' },
      { icon: Lightbulb, text: 'Ask the AI tutor to explain missed concepts step-by-step', color: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300' },
      { icon: Trophy, text: `Schedule a ${watchedSubject} mock exam for next week`, color: 'bg-danger-50 text-danger-700 dark:bg-danger-950/40 dark:text-danger-400' },
    ];
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                ...
                <div className="flex-1 min-w-0 flex items-center gap-5 sm:items-start">
                  <div className="relative shrink-0">
                    <svg width="180" height="180" className="transform -rotate-90">
                      <circle cx="90" cy="90" r="90" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="12" />
                      <motion.circle
                        cx="90"
                        cy="90"
                        r="90"
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <span className="text-primary-100 text-sm mt-1">Score</span>
                    </div>
                  </div>
                  <div className="flex-1 min-w-0 text-center sm:text-left">
                    <div className="flex items-center gap-2 justify-center sm:justify-start">
                      <span className={`inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r ${rank.color} px-4 py-1.5 text-sm font-bold text-white shadow-lg`}>
                        <Trophy className="h-4 w-4" /> {rank.tier} Rank
                      </span>
                    </div>
                    <h1 className="mt-3 text-2xl sm:text-3xl font-bold">
                      {scorePct >= 75 ? 'Outstanding performance!' : scorePct >= 50 ? 'Good effort — keep pushing!' : 'Time to review and try again'}
                    </h1>
                    <p className="mt-2 text-primary-100">
                      {watchedSubject} · {watch('topic')} · {questions.length} questions
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard title="Final Score" value={`${scorePct}%`} description={rank.tier + ' rank'} icon={<Trophy className="h-6 w-6" />} color="primary" />
          <StatCard title="Correct Answers" value={`${correct} / ${questions.length}`} description={`${questions.length - correct - skipped} wrong · ${skipped} skipped`} icon={<CheckCircle2 className="h-6 w-6" />} color="accent" />
          <StatCard title="Wrong Answers" value={wrong} description="Questions to review" icon={<XCircle className="h-6 w-6" />} color="danger" />
          <StatCard title="Time Taken" value={timeTaken || 'N/A'} description={examMode ? `of ${formatDuration(totalTime / 60)} total` : 'Estimated'} icon={<Clock className="h-6 w-6" />} color="secondary" />
        </div>
                    ...
                    {strongTopics.map((t) => (
                      <div key={t.topic}>
                        <div className="flex items-center justify-between text-sm mb-1">
                          ...
                          <span className="font-semibold text-accent-600 dark:text-accent-400">{t.score}%</span>
                        </div>
              ...
              {weakTopics.length > 0 && (
                <div>
                  <p className="text-sm font-semibold text-danger-700 dark:text-danger-400 mb-3 flex items-center gap-2">
                    <TrendingDownFake className="h-4 w-4" /> Weak Topics (Needs Work)
                  </p>
                  <div className="space-y-3">
                    {weakTopics.map((t) => (
                      <div key={t.topic}>
                        <div className="flex items-center justify-between text-sm mb-1">
                          <span className="text-slate-700 dark:text-slate-300">{t.topic}</span>
                          <span className="font-semibold text-danger-600 dark:text-danger-400">{t.score}%</span>
                        </div>
          ...
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <CardDescription>Personalized next steps based on your results</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {aiSuggestions.map((s, idx) => (
                <motion.div
                  ...
                  className={cn('flex items-start gap-3 rounded-xl p-4', s.color)}
                >
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <AnimatePresence mode="wait">
        {state === 'config' && (
          <div key="config">{renderConfigScreen()}</div>
        )}
        {state === 'quiz' && (
          <div key="quiz">{renderQuizScreen()}</div>
        )}
        {state === 'review' && (
          <div key="review">{renderReviewScreen()}</div>
        )}
        {state === 'results' && (
...
const TrendingUpFake: React.FC<{ className?: string }> = ({ className }) => (
...
const TrendingDownFake: React.FC<{ className?: string }> = ({ className }) => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
