...
import { STORAGE_KEYS, NOTIFICATION_SUBTYPES, ROUTES } from '@/constants';
...
const ICON_BY_SUBTYPE: Record<string, React.ComponentType<{ className?: string }>> = {
...
const COLOR_BY_TYPE: Record<string, string> = {
  success: 'bg-accent-50 text-accent-600 dark:bg-accent-950/40 dark:text-accent-400',
  error: 'bg-danger-50 text-danger-600 dark:bg-danger-950/40 dark:text-danger-400',
...
const SEED_NOTIFICATIONS: Notification[] = [
  ...
  },
  {
    ...
    message: 'Corrupted_File.docx could not be processed. The file appears damaged.',
    read: false,
  ...
  },
  {
    ...
    message: '32 cards from Physics: Waves & Oscillations are ready for review.',
    read: true,
  ...
  },
  {
    ...
    message: 'Your question about Schrödinger equation has a new detailed response.',
    read: true,
  ...
  },
  {
    ...
    message: 'Working offline. Your data is preserved locally until reconnected.',
    read: true,
...
const SIMULATED_NEW: Array<Omit<Notification, 'id' | 'createdAt' | 'read'>> = [
  {
    type: 'success',
...
export const NotificationCenter: React.FC<NotificationCenterProps> = ({ open, onClose, triggerRef }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const pick = SIMULATED_NEW[Math.floor(Math.random() * SIMULATED_NEW.length)];
  ...
  const renderCTA = (n: Notification) => {
    switch (n.subtype) {
      case 'upload.completed':
        return (
          <Button size="sm" variant="primary" onClick={() => handleAction(n)} rightIcon={<ExternalLink className="h-3.5 w-3.5" />}>
            View in Library
          </Button>
        );
      case 'upload.failed':
        return (
          <Button size="sm" variant="outline" onClick={() => toast.info('Retrying upload...')}>
            Retry
          </Button>
        );
      case 'quiz.generated':
        return (
          <Button size="sm" variant="primary" onClick={() => handleAction(n)} rightIcon={<ChevronRight className="h-3.5 w-3.5" />}>
            Start Quiz
          </Button>
        );
      ...
      case 'planner.generated':
        return (
          <Button size="sm" variant="outline" onClick={() => handleAction(n)} rightIcon={<ChevronRight className="h-3.5 w-3.5" />}>
            View
          </Button>
        );
      case 'ai.ready':
      ...
      case 'login.success':
        return (
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="danger" onClick={() => toast.info('Other sessions logged out')}>
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-4 py-3">
        ...
        <div className="flex items-center gap-1">
          ...
          {isMobile && (
            <button type="button" onClick={onClose} className="btn-ghost h-8 w-8 ml-1" aria-label="Close">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
      ...
      <div className="space-y-3 border-b border-slate-200 dark:border-slate-800 px-4 py-3">
        <div className="relative">
          <Search className="pointer-events-none absolute inset-y-0 left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" />
          <input
            type="search"
            value={search}
              ...
              return (
                <li
                  ...
                  className={cn(
                    ...
                    !n.read && 'border-l-[3px] border-l-primary-500 bg-primary-50/30 dark:bg-primary-950/20'
                  )}
                >
                  {isConfirmingDelete ? (
                    ...
                    <div className="flex items-start gap-3">
                      <div className={cn('mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl', COLOR_BY_TYPE[n.type])}>
                        <Icon className="h-4.5 w-4.5" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <p className={cn('text-sm', !n.read ? 'font-semibold text-slate-900 dark:text-slate-100' : 'font-medium text-slate-800 dark:text-slate-200')}>
                            {n.title}
                          </p>
...
export const NotificationBell: React.FC<NotificationBellProps> = ({ onClick, unreadCount, hasNewUnread }) => {
  return (
    <button
      type="button"
      onClick={onClick}
